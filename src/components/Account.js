...
export default class Home extends Component{
    ...
    fetchData(){
        //var ind = this.props.mail.indexOf('@')
        //var e = this.props.mail.slice(0,ind)
        firebase.database().ref('/'+'products/').once('value',(snapshot) => {
            db = snapshot.val();
            dt = this.state.data;
            l = [ ];
            for(c = 1 ;c <= db['count']['c'];c++){
                k = db['p' + c];
                k['key'] = c;
                if(k['contact'] == this.state.email){
                    dt.push(k);
                }
            }
    ...
    render() {
        ...
        return ( 
            <View style={styles.container}>
                { this.state.menuPressed ? ( 
                    <View style={styles.menuContainer}>
                        ...
                        <View style={styles.menuItemsContainer}>
                            ...
                            <TouchableOpacity style={styles.menuItem} onPress={()=>{Actions.Orders({mail:this.state.email})}}>
                ...
                <TouchableHighlight style={styles.buttonContainer} onPress={() => Actions.UploadForm({email:this.props.mail})}>
                        <Text style={styles.signUpText}>Sell a product</Text>
                </TouchableHighlight>
            </View>
        );
    }
